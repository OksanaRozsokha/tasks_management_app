...
void main() {
  ...
  TasksService tasksService = _connectToService();
  tasksService.addTask(TaskEntity(title: 'Hey', isCompleted: false));
  Future<List<TaskEntity>> tasksFut = _getTasks(tasksService);
  tasksFut.then((value) {
    print(value[0].title);
    tasksList = value;
    print(tasksList);

    tasksService.addTask(TaskEntity(isCompleted: true, title: 'My first completed task'));
    tasksList[0].setTitle('--------____--------');
    tasksService.updateTask(tasksList[0]);
    tasksService.removeTask(tasksList[0]);
    print(value);
    print(tasksList);
  });
  print('tasks are here ${tasksFut}');
  // print(tasksList);
...
_getTasks(TasksService service)  {
  return service.getAllTasks();
}

// Future getAllTasksEntities() async {
//    print('gfg');
//   dynamic tasksListJson = await rootBundle.loadString('assets/tasks_list.json');
//   print('___________-_-_______1');
//   final result = await json.decode(tasksListJson);
//   print('___________-_-_______');
//   print(result);
